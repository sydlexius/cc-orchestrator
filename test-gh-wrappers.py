...
def run_wrapper(wrapper, args, extra_env=None, grep_shim=False):
    """Run a wrapper with a stubbed `gh` on PATH. Returns (rc, gh_invocation_str).
    ...
    invoked = open(log).read().strip() if os.path.exists(log) else ""
    return p.returncode, invoked
...
def main():
    ...
    check("gh-codeql-dismiss valid -> PATCH the code-scanning alerts endpoint",
          rc == 0 and "api -X PATCH repos/o/r/code-scanning/alerts/7" in invoked)
    check("gh-codeql-dismiss endpoint is alerts-only (construction guarantee: no /merge)", "/merge" not in invoked)
    ...
    check("gh-codeql-dismiss accepts an enum reason", rc == 0 and "dismissed_reason=false positive" in invoked)
    ...
    check("gh-resolve-thread valid -> fixed resolveReviewThread graphql mutation",
          rc == 0 and "graphql" in invoked and "resolveReviewThread" in invoked)
    check("gh-resolve-thread cannot merge (construction guarantee: no pulls/merge)",
          "/merge" not in invoked and "pulls" not in invoked)
    ...
    check("gh-comment post valid -> POST issues/<pr>/comments",
          rc == 0 and "api -X POST repos/o/r/issues/12/comments" in invoked and "body=hello world" in invoked)
    check("gh-comment post construction guarantee (no /merge)", "/merge" not in invoked)
    ...
    check("gh-comment post body 'pulls/1/merge -X PUT --admin' is data only (endpoint+method fixed)",
          rc == 0 and invoked.startswith("api -X POST repos/o/r/issues/12/comments ")
          and "body=pulls/1/merge -X PUT --admin" in invoked)  # body rides as data, not the path/verb
    ...
    check("gh-comment reply valid -> POST pulls/<pr>/comments/<id>/replies",
          rc == 0 and "api -X POST repos/o/r/pulls/12/comments/987/replies" in invoked)
    check("gh-comment reply construction guarantee (no /merge)", "/merge" not in invoked)
    ...
    check("gh-comment trigger-cr valid -> POST issues comment with fixed CR body",
          rc == 0 and "api -X POST repos/o/r/issues/12/comments" in invoked
          and "body=@coderabbitai review" in invoked)
    ...
    check("gh-comment inline valid -> POST pulls/<pr>/comments with body+commit_id+path+line+side",
          rc == 0 and "api -X POST repos/o/r/pulls/12/comments" in invoked
          and "body=looks off" in invoked and "commit_id=" in invoked
          and "path=src/a.py" in invoked and "line=42" in invoked and "side=RIGHT" in invoked)
    check("gh-comment inline construction guarantee (no /merge, no --admin)",
          "/merge" not in invoked and "--admin" not in invoked)
    rc, invoked = run_wrapper("gh-comment.sh", ["inline", "12", "--file", "a.py", "--line", "5", "--side", "LEFT", "x"], env)
    check("gh-comment inline honors --side LEFT", rc == 0 and "side=LEFT" in invoked)
    ...
    check("gh-codeql-autofix valid -> POST code-scanning alerts autofix endpoint",
          rc == 0 and "api -X POST repos/o/r/code-scanning/alerts/7/autofix" in invoked)
    check("gh-codeql-autofix endpoint is alerts-only (construction guarantee: no /merge)", "/merge" not in invoked)
    ...
    check("gh-codeql-autofix accepts [repo] positional arg",
          rc == 0 and "repos/owner2/repo2/code-scanning/alerts/7/autofix" in invoked)
    ...
    check("gh-delete-branch valid -> DELETE git/refs/heads/<branch>",
          rc == 0 and "api -X DELETE repos/o/r/git/refs/heads/feat/24-gh-api-wrappers" in invoked)
    check("gh-delete-branch construction guarantee (no /merge)", "/merge" not in invoked)
    ...
    check("gh-delete-branch accepts [repo] positional arg",
          rc == 0 and "repos/owner2/repo2/git/refs/heads/topic" in invoked)
    # HOSTILE: a traversal repo arg must be REJECTED before interpolation.
