...
- **`thread-blocked head=...`** + Exit 0 -> if `failing=` is nonzero, fix CI first (handle-review's thread replies do not fix a red check); if `by=` names no `[bot]` login, the threads are human-opened and need the maintainer; otherwise invoke `/handle-review <pr>` (reply to and resolve the unresolved threads; a round that closes threads by reply-and-resolve may make no commit).
