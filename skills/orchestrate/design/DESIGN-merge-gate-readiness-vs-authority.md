...
Date: 2026-07-10
Status: DIRECTION APPROVED (maintainer, 2026-07-10) - design-of-record; pre-implementation.
The floor-touching slice (Piece B) still MUST survive an independent hostile pass + K=2 dry
rounds before implementation; see Rigor.
Issue: #262
Companion:
...
to fix a broken-copy interaction). So the escape hatch - "merge from a separate plain terminal" - costs a window switch AND a broken copy. **"Frictionless" here must mean: no window switch,
...
1. **There is no un-forgeable in-session human signal** (the `!` bash-mode escape *might* be one - a bot cannot type `!` - but whether `!` routes through the PreToolUse floor is undocumented;
...
`orchestrate-authorize-merge.sh 262` (→ hardened preflight → token) then
...
- [x] Tier-scoping decided (tier-agnostic authority; readiness carries the tier differences) - see "Decisions".
- [ ] Scoped `ship-gate-preflight` hardening review (exit-path audit; no spurious PASS on
  lookup error / partial response / adversarial input; fail-closed on unreadable state) - scoped here as Piece A(3); EXECUTED in the impl PR.
...
- [x] A linked implementation issue referencing this doc - filed as #263 (its floor-touching
  PR merges only after maintainer sign-off of the direction).
