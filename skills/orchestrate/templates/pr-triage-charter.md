...
- `${CLAUDE_PLUGIN_ROOT}/scripts/pr-watch.sh <pr> <REPO> <TIMEOUT>` IN THE BACKGROUND (run_in_background: true), then yield. Re-invoked on the event. No foreground-block, no gh-poll loop. BRANCH ON THE EXIT CODE (see SKILL.md): `0` = settled/blocked -> triage; `1` = timeout -> relaunch the background watch under the relaunch cap defined in SKILL.md AND send the lead a one-line heartbeat ("still watching #N, CR not posted, attempt k/cap") so a wait is never mistaken for a stall; `2` = setup-error -> STOP and message the lead, NEVER retry on `2` (a `2` usually means a bad arg -- a filesystem path where the `owner/name` slug belongs, or a missing PR). A `2` means the watch could not look; it NEVER means the PR merged/closed or that there is nothing to review - escalate, do not conclude "done".
- Read tool on repo files (to confirm a finding is real). Write to <OUTDIR> only. `mkdir -p <OUTDIR>`.
- `${CLAUDE_PLUGIN_ROOT}/scripts/pr-unreplied-comments.sh <PR#> <REPO>` (read-only) - the AUTHORITATIVE unreplied-feedback check (see "What to do per PR" step 1). NEVER pass `--latest-per-reviewer` on a merge-readiness check.
- `${CLAUDE_PLUGIN_ROOT}/scripts/pr-read-comments.sh <PR#> [<REPO>]` (read-only) - read the FULL comment + review BODIES behind the unreplied count, to confirm a finding is real before drafting. These two READ helpers (plus `pr-watch.sh`) are the ONLY scripts you run; the MUTATING reply/resolve helpers (`reply-comment.sh`, `resolve-threads.sh`) are the LEAD's - you DRAFT the reply text, the lead POSTS it (see SKILL.md "Lead PR tooling"). (These helpers are BUNDLED in the plugin under `scripts/`; the lead must hand you the resolved scripts dir - `${CLAUDE_PLUGIN_ROOT}` is NOT auto-substituted in a charter prompt, so the path you actually run is whatever the lead resolved it to when spawning you.)
...
1. Enumerate EVERY finding: CodeRabbit inline + review-body, Copilot, Greptile, CodeQL. Capture the comment/thread IDs. The AUTHORITATIVE enumeration of unaddressed feedback is `${CLAUDE_PLUGIN_ROOT}/scripts/pr-unreplied-comments.sh <PR#> <REPO>` - run it, do NOT hand-roll a comments-API walk (a hand-rolled walk reads only inline comments and MISSES review BODIES, where CodeRabbit carries its `Outside diff range comments (N)` Majors). It iterates ALL reviews, so a Major in a non-latest COMMENTED review is surfaced even when the LATEST review is APPROVED (an APPROVED review does NOT clear an outside-diff finding in another review). NEVER pass `--latest-per-reviewer` on a merge-readiness check - it re-hides exactly that non-latest Major (dogfood: stillwater#1931). Reply to an outside-diff finding via the review-body context (it has no resolvable inline thread). The same all-reviews rule applies to INLINE comments: CR can SPLIT actionable items across MULTIPLE review submissions (two `Actionable comments posted: N` bodies), so your finding set is the tool's FULL inline list aggregated across EVERY submission - never just one review's comments (dogfood stillwater#200: triage processed one of two CHANGES_REQUESTED submissions and silently dropped two Major inline findings). COUNT SELF-CHECK (completeness proof): the SUM of every CR review's `Actionable comments posted: N` must reconcile with `(inline unreplied + review-body actionable)` you enumerate; a shortfall means you dropped a submission - re-pull before reporting, never report the partial set.
