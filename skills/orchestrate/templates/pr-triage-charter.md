...
- `~/.claude/scripts/pr-watch.sh <pr> <REPO> <TIMEOUT>` IN THE BACKGROUND (run_in_background: true), then yield. Re-invoked on the event. No foreground-block, no gh-poll loop. BRANCH ON THE EXIT CODE (see SKILL.md): `0` = settled/blocked -> triage; `1` = timeout -> relaunch the background watch under the relaunch cap defined in SKILL.md AND send the lead a one-line heartbeat ("still watching #N, CR not posted, attempt k/cap") so a wait is never mistaken for a stall; `2` = setup-error -> STOP and message the lead, NEVER retry on `2` (a `2` usually means a bad arg -- a filesystem path where the `owner/name` slug belongs, or a missing PR). A `2` means the watch could not look; it NEVER means the PR merged/closed or that there is nothing to review - escalate, do not conclude "done".
- Read tool on repo files (to confirm a finding is real). Write to <OUTDIR> only. `mkdir -p <OUTDIR>`.
- `~/.claude/scripts/pr-unreplied-comments.sh <PR#> <REPO>` (read-only) - the AUTHORITATIVE unreplied-feedback check (see "What to do per PR" step 1). NEVER pass `--latest-per-reviewer` on a merge-readiness check.
- `~/.claude/scripts/pr-read-comments.sh <PR#> [<REPO>]` (read-only) - read the FULL comment + review BODIES behind the unreplied count, to confirm a finding is real before drafting. These two READ helpers (plus `pr-watch.sh`) are the ONLY scripts you run; the MUTATING reply/resolve helpers (`reply-comment.sh`, `resolve-threads.sh`) are the LEAD's - you DRAFT the reply text, the lead POSTS it (see SKILL.md "Lead PR tooling"). (These helpers live in `~/.claude/scripts/` today, same as `pr-watch.sh` above; the #30 plugin will path-resolve them.)
