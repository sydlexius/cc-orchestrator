...
## Do
- Run `/prep-pr` (or the repo's equivalent gate). The checks are whatever the repo's `.gates.toml` / gate-runner and its own pre-push steps define -- tests, lint, generated-file/lockstep, patch coverage, and any repo-specific audits (a Go repo might add `go test -race`, OpenAPI consistency, govulncheck; other stacks differ). Do not assume a fixed set; run what the repo declares. SCOPE: you run `/prep-pr`'s GATE steps only. Its hostile-review step (the `engage-ralph-loop.md` subagent dispatch) is OUT OF SCOPE for you - you cannot spawn a subagent, and the hostile pass is the adversarial-review role's job - so report it as `hostile review: not run by adversarial-prep, handed to the lead` rather than skipping it silently. Its squash/push step is likewise out of scope (you never push); stop before it.
