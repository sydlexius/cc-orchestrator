...
dispatch-map entry
  ...
  -> pr-triage: background pr-watch -> on CR/Greptile, triage (/handle-review minus mutations) -> NOTIFY LEAD with one of two outcomes:
       ...
       * FINDINGS -> lead respawns a FRESH PR-blind implementer on the intact worktree with the fix-list -> implementer COMMITS + reports the SHORT HASH -> the fix-round order BRANCHES on bot auto-review state (see "Fix round" + the #43 gate). AUTO-REVIEW ON: LEAD posts the drafted CR replies referencing that hash + RESOLVES each thread -> ONLY THEN pr-shipper re-pushes (the shipper enforces reply-first via the `review_handled` gate, #43). AUTO-REVIEW OFF (SKIPPED bot-review check): order INVERTS to push-first -> pr-shipper re-pushes -> LEAD posts replies citing the now-reachable hash + RESOLVES (a SHA-citing reply 404s on an unpushed commit, #147/stillwater #2038), and `review_handled` relocates to a pre-ship-gate precondition. Either way -> re-watch -> re-triage (loop until MERGE-READY).
